...
**Title format (every entry).** A sentence-style news headline, ≤ ~90 characters (hard cap 110). The named work, site, or institution appears early. Event entries take a documented-action verb — name the actor only when the action is undisputed ("Swiss museums return 18 Benin Bronzes to Nigeria"); where responsibility is contested, use the passive or name the dispute ("Reflecting Pool paint fails; administration alleges vandalism"). No verdict adjectives; characterizations that would need attribution are omitted — titles have no room to attribute. Umbrella parents tracking a phenomenon rather than an event take a descriptive noun phrase naming the scope ("Federal 'beautification' program and the National Mall"). Sub-entries: parent shorthand, colon, development headline. "N/A" never appears in a title — it belongs to the artist/institution fields.
...
**Washington Post standing search (daily):** Run `site:washingtonpost.com after:[DATE-48H]` with the DC federal-cultural cluster terms above — National Mall, Reflecting Pool, America 250, triumphal arch, Smithsonian, NEA, NEH, GSA fine arts, Cultural Landscape Foundation, White House ballroom, East Wing — plus any active case names from the 046 / 005 / 050 / 069 cluster (the 050 White House ballroom / East Wing thread is a live, court-driven case — check it every day it stays unresolved). Substitute the actual date 48 hours before the run for [DATE-48H] (e.g. `after:2026-06-27` for a June 29 run). **The date filter is mandatory** — without it the search surfaces prominent older articles rather than breaking coverage. WaPo is the primary outlet for National Mall, federal arts institution, and America 250 coverage; this sweep runs every day regardless of trawl region. Supplements (does not replace) the general dragnet.
...
- **Wednesday — Europe, UK, Francophone Africa** (French, German, Italian, Dutch, Spanish-for-Spain, Hungarian). EU national/regional outlets; French-language African outlets (Senegal, Côte d'Ivoire, DRC, Cameroon). Look for: repatriation disputes, deaccessioning, memorial controversies, EU cultural policy, religious-objection cases, postcolonial disputes, human-remains repatriation.
...
*Evaluated, not excluded (determinations on record).* When a source's fitness is questioned and reviewed, the outcome is written here so the next scan doesn't re-litigate it. Being listed means the opposite of exclusion: the source was checked against the exclusion bar, did not meet it, and stays in the default disposition above (cite when corroborated or when carrying attributed original reporting; never a sole basis for a contested claim). This is not a trust rating and confers no automatic weight.
- **The Washington Times** (reviewed 2 July 2026) — conservative, Unification-movement-owned, right bias with a "mixed" independent factual rating and a history of false/fringe claims in specific areas (notably climate science). **Does not meet the exclusion bar:** unlike Breitbart and the Daily Mail it is not Wikipedia-deprecated, and it does real original DC political/courts reporting. The gap to Breitbart is categorical (excluded vs. includable), not a step on a spectrum. Handle per-story: fine when corroborated or attributed as its own reporting; not a sole source for a contested political claim.
- **Urgent Matter** (urgentmatter.press; reviewed 28 August 2026) — single-author art-law newsletter by journalist Adam Schrader (prior bylines at ARTnews and UPI; earlier tabloid work at The Sun / Page Six), launched 2025, working substantially from primary court documents. **Does not meet the exclusion bar** — no fabrication record, and document-based reporting is inherently verifiable. But as a young outlet with a single editorial voice it has no established track record yet. **Posture: a corroborated / secondary source, and citable for its own attributed primary-document reporting; not a sole source for a contested claim, pending track record.** Artforum has cited and linked it — a positive signal from an established desk. Revisit as the track record develops.
- **Artlyst** (artlyst.com; reviewed 30 August 2026) — London-based independent contemporary-art news and reviews site, publishing ~15 years, with a named UK/international team and bylined coverage. **Does not meet the exclusion bar** — no fabrication record, no deprecation. An established trade outlet rather than a young one, but a smaller, partly aggregating art-press site rather than a primary newsgathering desk, so **posture: cite when corroborated or when carrying its own attributed reporting; fine as a full-text reference source where a paywalled originator (e.g. NYT) broke the story, provided the facts are corroborated.** First used as the reference source for ACI-345 (Kehinde Wiley embassy removal), corroborated across NYT/ARTnews/Artforum/TAN and others.
...
- **Mockery is not claimsmaking.** Ridicule, derision, or "shade" toward an artwork or public object — even widespread, and even from named public figures — does not by itself qualify. Inclusion requires a *claim*: on the object (that it fails as art, is an affront, does not belong in its space) or on a party's status or authority. That people found something laughable is not a controversy.
...
- **Backfill dating.** When a current scan surfaces a controversy whose origin predates the scan, anchor the parent to the controversy's origin, not the day it was discovered (`date_discovered` still records the scan date). Treat each historical moment as a moment of *contemporary* news coverage: source the parent from the outlet(s) that broke the story at the time, `sort_date` to that reporting, and draft narrowly from it — do not substitute a later retrospective or scholarly summary where contemporaneous journalism exists. A later development in the same controversy (a ruling, an appeal, a sale) is its own dated sub-entry when it, too, drew contemporary coverage; where a development drew **no** press (e.g., an appeal decided only in the law reports), fold it into the parent's outcome, cite the primary/court record, and note the absence of coverage — silence in the press is itself a coverage-pattern data point, not evidence the development did not happen. (See ACI-387 / -A, ACI-390 / -A, ACI-391, ACI-392 / -A / -B.)
